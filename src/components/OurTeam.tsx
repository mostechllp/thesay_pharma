...
export function OurTeam() {
  return (
    <section className="team-section pt-[120px] px-[30px] pb-[60px] overflow-hidden relative z-10 bg-white">
      ...
      <div className="container mx-auto px-4 md:px-8">
        ...
        <div className="sec-title mb-[50px]">
          <div className="flex justify-between items-center flex-wrap gap-4">
            <motion.h2 
              className="title text-3xl md:text-4xl font-bold text-[#0f3d2e] tracking-wide"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {team.map((member, index) => (
            ...
            >
              <div className="inner-box relative z-10 transition-all duration-300 rounded-[20px] bg-white hover:bg-[#27ae3f]">
                {/* Image */}
                <figure className="image rounded-t-[20px] overflow-hidden transition-all duration-300 relative">
                  <img 
                    ...
                    alt={member.name} 
                    loading="lazy"
                    className="w-full aspect-square object-cover transition-all duration-300 hover:grayscale"
                  />
                  {/* Gradient Overlay - appears on hover */}
                  <div className="absolute bottom-0 left-0 w-full h-full bg-gradient-to-t from-[#0f3d2e] to-transparent opacity-0 invisible transition-all duration-300 group-hover:opacity-100 group-hover:visible"></div>
                </figure>
      ...
      {/* Add custom CSS for group hover */}
      <style jsx>{`
